...
def calculate_goalpoint_manhattan(laser_data):
    import math
    rendp, rangle = get_rightwall_endpoint(laser_data)
    right_end = lidar_to_rear_axle_coords(rendp)
    dist_to_right_wall = get_distance_at_angle(laser_data, -90)
    lendp, langle = get_leftwall_endpoint(laser_data)
    left_end = lidar_to_rear_axle_coords(lendp)
    dist_to_left_wall = get_distance_at_angle(laser_data, 90)
    if math.fabs(math.degrees(rangle)) > 75:
        goalpoint_at_infinity = left_end/2
    elif math.fabs(math.degrees(langle)) > 75:
        goalpoint_at_infinity = right_end/2
    else:
        goalpoint_at_infinity = (right_end + left_end)/2 #right_bias*right_end + left_bias*left_end) / 3
...
def calculate_goalpoint_double_manhattan(laser_data):
    ...
    #left_end_angle_rad = math.atan(left_end[1] / left_end[0])
    #left_end_2 = lidar_to_rear_axle_coords(get_second_leftwall_endpoint(laser_data, left_end_angle_rad))
    #left_end_2 *= .4
    #left_end_angle = math.degrees(left_end_angle_rad)
    #if left_end_angle > 0:
    goalpoint_at_infinity = (right_end + left_end + right_end_2) / 2.4
    #else:
...
def calculate_goalpoint_big_gap(laser_data):
    def gap(point1, point2):
        import numpy as np
        return np.sqrt((point1[0]-point2[0])**2 + (point1[1]-point2[1]))
    max_gap, max_gap_point = 0, np.array([0, 0])
    for index,range in enumerate(laser_data.ranges):
        if index >=1:
            this_gap = gap(get_point_at_index(laser_data, index), get_point_at_index(laser_data, index-1))
            if this_gap > max_gap:
                max_gap = this_gap
                max_gap_point = (get_point_at_index(laser_data, index) +
                                 get_point_at_index(laser_data, index-1)) / 2
    return max_gap_point