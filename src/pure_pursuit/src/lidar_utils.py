...
pub_leftwall_2 = rospy.Publisher('leftwall/wall2', PolygonStamped, queue_size=1)
pub_leftwall_endpoint_2 = rospy.Publisher('leftwall/endpoint2', PointStamped, queue_size=1)
...
def get_distance_at_angle(laser_data, angle_deg):
    ...
    try:  # Fuzzy range
        distance_to_lidar = laser_data.ranges[index-2] + laser_data.ranges[index] + laser_data.ranges[index+2]
        distance_to_lidar = distance_to_lidar/3
    except:
...
def get_angle_at_lidar_index(laser_data, index):
    angle_float = np.interp(index, [0, len(laser_data.ranges) - 1],
                            [math.degrees(laser_data.angle_min), math.degrees(laser_data.angle_max)])
    return angle_float


def get_point_at_index(laser_data, index):
    return get_lidar_point_at_angle(laser_data, get_angle_at_lidar_index(laser_data, index))
...
def get_rightwall_endpoint(laser_data):
    ...
    p2 = get_lidar_point_at_angle(laser_data, -90+10)
    delta_y = p2[1] - p1[1]
    delta_x = p2[0] - p1[0]
    ...
    print("RAngle is:", math.degrees(angle))
...
def get_leftwall_endpoint(laser_data):
    ...
    angle = math.atan(delta_y / delta_x)
    print("LAngle is:", math.degrees(angle))

    return endpoint, angle


def get_second_leftwall_endpoint(laser_data, wall_angle_start):
    p1 = get_lidar_point_at_angle(laser_data, math.degrees(wall_angle_start))
    p2 = get_lidar_point_at_angle(laser_data, math.degrees(wall_angle_start)-10)
    delta_y = p2[1] - p1[1]
    delta_x = p2[0] - p1[0]
    ...
    endpoint_msg.header = Header(stamp=rospy.Time.now(), frame_id="laser")
    endpoint_msg.point = Point(endpoint[0], endpoint[1], 0)
    pub_leftwall_endpoint_2.publish(endpoint_msg)
